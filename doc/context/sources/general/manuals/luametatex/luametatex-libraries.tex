...
We also have regular float and double readers:

\starttyping[option=LUA]
function fio.readfloat    ( <t:file> handle ) return <t:number>  end
function fio.readfloatle  ( <t:file> handle ) return <t:number>  end
function fio.readdouble   ( <t:file> handle ) return <t:number>  end
function fio.readdoublele ( <t:file> handle ) return <t:number>  end
\stoptyping
...
function fio.writefloat    ( <t:file> handle, <t:number> value ) end
function fio.writefloatle  ( <t:file> handle, <t:number> value ) end
function fio.writedouble   ( <t:file> handle, <t:number> value ) end
function fio.writedoublele ( <t:file> handle, <t:number> value ) end
\stoptyping
...
\starttyping[option=LUA]
function sio.readfloat    ( <t:file> handle ) return <t:number>  end
function sio.readfloatle  ( <t:file> handle ) return <t:number>  end
function sio.readdouble   ( <t:file> handle ) return <t:number>  end
function sio.readdoublele ( <t:file> handle ) return <t:number>  end
\stoptyping
...
A table with \UNICODE\ slots can be converted to an \UTF\ string with:

\starttyping[option=LUA]
function string.utftabletostring (
    <t:table>   t,
    <t:integer> first, -- optional
    <t:integer> last   -- optional
)
    return <t:string>
end
\stoptyping

Strings can be split in many ways so in \CONTEXT\ we delegate that to \LUA,
but we do provide:

\starttyping[option=LUA]
function string.splitintolines ( <t:string> str )
    return <t:table>
end
\stoptyping

This function handles linefeed (\ASCII~10), carriage return (\ASCII~13) and the
sequence carriage return linefeed (\ASCII~13 followed by \ASCII~10).
...
\starttyping[option=LUA]
function pngdecode.applyfilter (
    <t:string>  data,
    <t:integer> nx,
    <t:integer> ny,
    <t:integer> slice
...
\starttyping[option=LUA]
function pngdecode.togray ( <t:string data )
    return <t:string>
end

function pngdecode.tocmyk ( <t:string data )
    return <t:string>
end
\stoptyping
...
A (large) bitmap can be reduced to half the resolution with the next helper.
Arbitrary reduction makes little sense so that was discarded.

\starttyping[option=LUA]
function pngdecode.reduce (
    <t:string> data,
    <t:string> nx,
    <t:string> ny,
    <t:string> nz
...
These are used to convert \PNG\ streams into streams that are suitable for \PDF\
inclusion or further manipulations. Keep in mind that \PDF\ (contrary to \JPEG\
inclusion) doesn't really embed a \PNG, but sees it as a compression scheme, so
masks and transparency are separated. In practice it also means that only zip
compression makes sense (which is natural to \PDF) so in \CONTEXT\ we always go
simple and compress like that. There is no benefit in the other filtering which
is why we only have decoders.

\starttyping[option=LUA]
function pngdecode.frompalette (
    <t:string> content,
    <t:string> palette,
    <t:string> bytes,
    <t:string> paths,
    ...
    return
        <t:string>, -- expanded
        <t:string>  -- mask (in case of transparent)
end

function pngdecode.palettemask (
    <t:string> content,
    <t:string> transparent,
    <t:string> bytes,
    <t:string> paths
...
function pngdecode.transparentmask (
    <t:string> content,
    <t:string> bytes,
    <t:string> depth,
    <t:string> transparent -- optional
)
    return <t:string>
end
\stoptyping

The \type {pdgdecode.setexperiment} option (bit 1) uses so call external strings
in \LUA\ which saves copies but is, as said, experimental.
...
For \LUAMETAFUN\ we have added some additional in pure \LUA; those are not
described here as they are not part of the engine.
...
\starttyping[option=LUA]
function xcomplex.abs   ( <t:complex> z ) return <t:number>  end
function xcomplex.arg   ( <t:complex> z ) return <t:number>  end
function xcomplex.imag  ( <t:complex> z ) return <t:number>  end
function xcomplex.real  ( <t:complex> z ) return <t:number>  end
function xcomplex.conj  ( <t:complex> z ) return <t:complex> end
...
We added the \type {cerf} functions but one can wonder if we should carry that
...
\startsubsection[title=Bitset]

This provides an efficient way to store a state and it's a by-product of
the zbuffering feature. The library defaults to one-based sets but we can
also be zero-based:

\starttyping[option=LUA]
function bitset.new (
    <t:integer> n,
    <t:boolean> dflt -- optional
)
    return <t:bitset>
end

function bitset.newonebased  (
    <t:integer> n,
    <t:boolean> dflt -- optional
)
    return <t:bitset>
...
These directly operate on a bitset:

\starttyping[option=LUA]
function bitset.set    ( <t:bitset> s ) end
function bitset.get    ( <t:bitset> s ) end
function bitset.assign ( <t:bitset> s, <t:boolean> value ) end
function bitset.reset  ( <t:bitset> s ) end
function bitset.wipe   ( <t:bitset> s ) end
\stoptyping

There are some serializers too:

\starttyping[option=LUA]
function bitset.asstring ( <t:bitset> s ) return <t:string> end
function bitset.tostring ( <t:bitset> s ) return <t:string> end
function bitset.totable  ( <t:bitset> s ) return <t:table>  end
\stoptyping

The nest repertoire does some juggling:

\starttyping[option=LUA]
function bitset.band ( <t:bitset> a, <t:bitset> b )
function bitset.bor  ( <t:bitset> a, <t:bitset> b )
function bitset.bxor ( <t:bitset> a, <t:bitset> b )
function bitset.bnot ( <t:bitset> a, <t:bitset> b )
function bitset.shl  ( <t:bitset> a, <t:bitset> b )
function bitset.shr  ( <t:bitset> a, <t:bitset> b )
function bitset.len  ( <t:bitset> s )
\stoptyping

These actions are also available as more direct operators so we can say \type {a
& b}, \type {a | b}, \type {a ~ b}, \type {~b}, \type {b << k}, \type {b >> k}
and even \type {#b}. The length operator is interesting because we can use of as
weight in an ordered set of options, locations, complexity etc. We'll see where
it gets applied in \CONTEXT.

Traversing over a set can be done in three ways and we can best show these with
examples:

\startbuffer
\startluacode

local bs = bitset.new(4,true)

context("astring : [%s]",bitset.asstring(bs))
context.par()
...
context("all :")
for n, b in bitset.traverse(bs) do
    context(" [%i=%l]",n,b)
end
context.par()

context("true :")
for n, b in bitset.traverse(bs,true) do
    context(" [%i]",n)
end
context.par()

context("false :")
for n, b in bitset.traverse(bs,false) do
    context(" [%i]",n)
end
context.par()
\stopluacode
...
Here we have the default one-based sets, so we get:
...
\startsubsection[title=Interval]

This library is only present for testing the concept and relates to the
(optional) interval number model in \METAPOST. We're not sure if this module
makes sense.

\stopsection

\startsubsection[title=Z-buffers]

We're still working on this one as it is part of the \LUAMETAFUN\ three
dimensional rendering. The functions are very much geared to the wrappers and
work with userdata objects: \type {zbuffer}, \type {points}, \type {meshes},
\type {triangles}, etc. It's a mix and trade off between flexible \LUA\ and fast
\CCODE.

\stopsection

\startsubsection[title=QR-codes]

We can generate various (bar) codes with \type {zint} either using the command
line program or using the optional library. However, this is a bit tricky
dependency because one need a version that matches our usage and interfaces.

In practice only a few codes matter in the perspective of typesetting. In
\CONTEXT\ we have various modules and one generates the most common EAN like
codes, but because QR codes are more complex we delegate that to a wrapper around
the small and effective \type {qrcodegen} library. The \LUA\ interface is just
one function with only the first argument being mandate:
...
In \CONTEXT\ we feed that into the \LUAMETAFUN\ bytemap handler which adds some
more options, like different shapes instead of (efficient) scaled square pixels.

\stopsection

\startsubsection[title=JPEG]

We can handle \PNG\ files with a mix of \LUA\ and \CCODE\ helpers which gives us
flexibility, performance and no need for a large library with platform specific
code paths and other complications. This also makes sense because in \PDF\
lossless bitmap graphics can be stored in what is called \PNG\ encoding.

However, \JPEG\ is a bit more complex. On the one hand we can directly embed the
blob, so again we use \LUA. The reason why we have provide an interface to the
\type {nanojpeg} library is that it permits us to load a (not too complex) \JPEG\
encoded image and use the various bytemap functionality to manipulate it in a
\METAPOST\ setting. Of course we then eventually end up with embedding a lossless
result (\PNG\ encoded).

\starttyping[option=LUA]
function nanojpeg.decode ( <t:string> jpegdata )
    return
        <t:integer>, -- width
        <t:integer>, -- height
        <t:integer>, -- colordepth
...
\startsection[title=KD trees]

This library is part of the z-buffering and voronoi support in \LUAMETAFUN\ and
might find more usage in future version of \CONTEXT. Here is the repertoire of
functions. Keep in mind that this is a very specific kind of storage, quite often
\LUA\ tables and hashes are more efficient.
...
function kdtree.newweighted (
    <t:table> data
)
    return <t:kdtree>
end

function kdtree.reset (
    <t:kdtree> tree
)
end

function kdtree.insert (
    <t:kdtree> tree,
    <t:number> x,
    <t:number> y,
    <t:number> z
...
function kdtree.totable (
    <t:kdtree> tree
)
    return <t:table> -- of { x, y, z }
end
\stoptyping
...
kdtree.insert(k,10,40) kdtree.insert(k,20,60)
kdtree.insert(k,30,20) kdtree.insert(k,31,10)

context("found=%s",tostring(kdtree.found(k,20,40))) context.par()
context("found=%s",tostring(kdtree.found(k,20,60))) context.par()
context("found=%s",tostring(kdtree.found(k,21,10))) context.par()

context("index=%i x=%N y=%N z=%N", kdtree.nearest(k,21,30))
context.par()

for index, x, y, z in kdtree.traverse(k) do
    context("index=%i x=%N y=%N z=%N",index,x,y,z)
    context.par()
end

local w = kdtree.newweighted {
    { 10, 40 },
    { 20, 60 },
    { 30, 20 },
    { 31, 10 },
...
context("index=%i x=%N y=%N z=%N", kdtree.nearestindex(w,21,30))
context.par()
\stopluacode
\stopbuffer
...
\startsubsection[title=Effects]

This is a relatively large library with various (noise) helpers that are used in
\LUAMETAFUN\ interfaces: Brownian, Perlin, simplex, octave, and Worley
operations. Some fallback on existing (published) reference implementations.

\stopsection

\startsubsection[title=Bytemaps]

This is a relatively large library with various helpers that are used in
\LUAMETAFUN\ interfaces, think byte, gray, \RGB\ allocation and access, clipping,
filtering, slicing, overlays, reduction, and conversion.

\stopsection

\startsubsection[title=Serial]

This module was introduced when we presented the \CONTEXT\ clock a circular
device that shows progress and status of a run. The interface is minimal:

\starttyping[option=LUA]
function serial.open ( <t:string> portname, <t:integer> baudrate )
    return <t:userdata> -- connection
end

function serial.close ( <t:userdata> connection )
    -- no return value
end

function serial.send ( <t:userdata> connection, <t:string> data )
    return <t:boolean> -- success
end
\stoptyping

These come together in the next helper, which of course is rather inefficient
because it opens and closes the post every time:
...
Examples can be found in the \CONTEXT\ distribution. The signaling device is
described in a manual and article.
...
\starttyping[option=LUA]
function optional.library.load (
    <t:string> filename,
    <t:string> openname,
)
    return
...
\starttyping[option=LUA]
function optional.graphicsmagick.getnoisetypes ( )
    return <t:table>
end
\stoptyping
...
I added this because we wanted to have something \type {ffi} as in \LUATEX, and
it runs on top of a runtime loaded \type {libffi}. The code is inspired by \quote
{alien}. Some examples are given in \typ {libs-imp-foreign.mkxl}. and when there
is demand we might document it in more detail here. Till then it is considered
experimental.
