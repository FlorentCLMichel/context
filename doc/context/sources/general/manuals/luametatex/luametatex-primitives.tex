...
This is the \UNICODE\ variant of \prm {delcode}: it sets or returns the small and
large delimiter codes associated with a character.

\stopnewprimitive

\startnewprimitive[title={\prm {Udelimited}}]

See \prm {Uradical} for an introduction.

\stopnewprimitive

\startnewprimitive[title={\prm {Udelimiter}}]

This creates a delimiter from a \UNICODE\ math character specification. Unlike a
fence made with \prm {Uleft}, it does not delimit a subformula.
...
This attaches one or more accent characters to a math nucleus. Options select
top, bottom or overlay accents and control how they adapt to the nucleus.
...
This is the depth of the base used when placing an accent. It complements \prm
...
This is the height of the base used when placing an accent.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathaccentbottomovershoot}}]

This is the amount by which a bottom accent can extend beyond the base.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathaccentbottomshiftdown}}]

This is the amount by which a bottom accent is shifted down.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathaccentextendmargin}}]

This is the margin added when an accent is automatically extended.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathaccentsuperscriptdrop}}]

This controls the drop applied to a superscript attached to an accented nucleus.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathaccentsuperscriptpercent}}]

This is the percentage used when positioning a superscript on an accented
nucleus.
...
This is the amount by which a top accent can extend beyond the base.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathaccenttopshiftup}}]

This is the amount by which a top accent is shifted up.
...
This modifier makes the current math atom adapt its size to the atom on its left.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathadapttoright}}]

This modifier makes the current math atom adapt its size to the atom on its
right.
...
This is the height of the math axis above the baseline.
...
This inserts a math character specified by a \UNICODE\ math code.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathchardef}}]

This defines a control sequence as a math character specified by a \UNICODE\ math
code.
...
This sets or returns the \UNICODE\ math code of a character, including its class
and family.
...
This is the minimum overlap between parts of an extensible math construction.
...
It is used for the degree in \prm {Uroot} and \prm {Urooted}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathdelimiterextendmargin}}]

This is the margin added when a delimiter is automatically extended.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathdelimiterovervariant}}]

The current value of this parameter is \CurrentMathVariant
\Umathdelimiterovervariant. It is used by \prm {Udelimiterover}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathdelimiterpercent}}]

This is the percentage used when sizing a delimiter for the current style.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathdelimitershortfall}}]

This is the amount by which a delimiter may fall short of the requested size.
...
The current value of this parameter is \CurrentMathVariant
\Umathdelimiterundervariant. It is used by \prm {Udelimiterunder}.
...
Like \prm {Umathchardef}, this defines a math character and also associates a
math dictionary with it.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathexheight}}]

This is the ex-height used by the current math style.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasubpreshift}}]

This adds a vertical shift to a sub-prescript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasubprespace}}]

This adds horizontal space before a sub-prescript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasubshift}}]

This adds a vertical shift to a subscript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasubspace}}]

This adds horizontal space before a subscript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasuppreshift}}]

This adds a vertical shift to a super-prescript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasupprespace}}]

This adds horizontal space before a super-prescript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasupshift}}]

This adds a vertical shift to a superscript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathextrasupspace}}]

This adds horizontal space before a superscript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathflattenedaccentbasedepth}}]

This is the depth of the flattened base used when placing a wide accent. It
complements \prm {Umathflattenedaccentbaseheight}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathflattenedaccentbaseheight}}]

This is the height of the flattened base used when placing a wide accent.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathflattenedaccentbottomshiftdown}}]

This is the amount by which a wide bottom accent is shifted down.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathflattenedaccenttopshiftup}}]

This is the amount by which a wide top accent is shifted up.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathfractiondelsize}}]

This is the minimum delimiter size used by a fraction with delimiters.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathfractiondenomdown}}]

This is the downward shift of the denominator in a fraction.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathfractiondenomvgap}}]

This is the vertical clearance between the denominator and the fraction rule.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathfractionnumup}}]

This is the upward shift of the numerator in a fraction.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathfractionnumvgap}}]

This is the vertical clearance between the numerator and the fraction rule.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathfractionrule}}]

This is the thickness of the rule in a fraction.
...
The current value of this parameter is \CurrentMathVariant
\Umathhextensiblevariant. It is used by \prm {Uhextensible}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathlimitabovebgap}}]

This is the baseline clearance for a limit placed above an operator.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathlimitabovekern}}]

This is the space reserved at the top of a limit placed above an operator.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathlimitabovevgap}}]

This is the vertical clearance for a limit placed above an operator.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathlimitbelowbgap}}]

This is the baseline clearance for a limit placed below an operator.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathlimitbelowkern}}]

This is the space reserved at the bottom of a limit placed below an operator.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathlimitbelowvgap}}]

This is the vertical clearance for a limit placed below an operator.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathlimits}}]

This modifier places scripts on top of and below the current operator.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathnoaxis}}]

This modifier prevents the current math atom from being aligned on the math axis.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathnolimits}}]

This modifier forces scripts on the current operator to remain as postscripts.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathnolimitsubfactor}}]

This is the multiplier used when shifting a subscript on an operator without
limits.
...
This is the multiplier used when shifting a superscript on an operator without
limits.
...
This modifier adds the given depth to the current math atom.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathopenupheight}}]

This modifier adds the given height to the current math atom.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathoperatorsize}}]

This is the minimum size of a large operator in display style.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathoverbarkern}}]

This is the vertical clearance above the rule in an overbar.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathoverbarrule}}]

This is the thickness of the rule in an overbar.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathoverbarvgap}}]

This is the vertical clearance below the rule in an overbar.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathoverdelimiterbgap}}]

This is the baseline clearance used by \prm {Uoverdelimiter} and \prm
{Udelimiterover}.
...
The current value of this parameter is \CurrentMathVariant
\Umathoverdelimitervariant. It is used by \prm {Uoverdelimiter}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathoverdelimitervgap}}]

This is the vertical clearance used by \prm {Uoverdelimiter} and \prm
{Udelimiterover}.
...
This modifier discards the content of the current atom but retains its
dimensions.
...
This is the percentage by which a prime is raised.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathprimeraisecomposed}}]

This is the percentage by which a composed prime is raised.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathprimeshiftdrop}}]

This is the maximum baseline drop applied to a prime.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathprimeshiftup}}]

This is the upward shift applied to a prime.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathprimespaceafter}}]

This is the additional space inserted after a prime.
...
This is the width of eighteen math units in the current style.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathradicaldegreeafter}}]

This is the horizontal kern after the degree in \prm {Uroot} and \prm {Urooted}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathradicaldegreebefore}}]

This is the horizontal kern before the degree in \prm {Uroot} and \prm {Urooted}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathradicaldegreeraise}}]

This is the percentage by which the degree is raised in \prm {Uroot} and \prm
{Urooted}.
...
This is the horizontal space after an extensible radical symbol in \prm
{radical}, \prm {Uradical}, \prm {Uroot} and \prm {Urooted}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathradicalextensiblebefore}}]

This is the horizontal space before an extensible radical symbol in \prm
{radical}, \prm {Uradical}, \prm {Uroot} and \prm {Urooted}.
...
This is the vertical clearance above the rule in \prm {radical}, \prm {Uradical},
\prm {Uroot} and \prm {Urooted}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathradicalrule}}]

This is the thickness of the rule in the radical and related delimiter
constructors.
...
It selects the style used for the radicand in the radical constructors.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathradicalvgap}}]

This is the vertical clearance between the radicand and the rule in \prm
{radical}, \prm {Uradical}, \prm {Uroot} and \prm {Urooted}.
...
This is the default depth of a math strut rule when no depth is specified.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathruleheight}}]

This is the default height of a math strut rule when no height is specified.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathskeweddelimitertolerance}}]

This is the tolerance used when sizing a delimiter in a skewed fraction.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathskewedfractionhgap}}]

This is the horizontal gap used when positioning the parts of a skewed fraction.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathskewedfractionvgap}}]

This is the vertical gap used when positioning the parts of a skewed fraction.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsource}}]

This modifier assigns a source identifier to the current math atom or its
nucleus.
...
This is the additional horizontal space inserted after a super- or subscript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathspacebeforescript}}]

This is the additional horizontal space inserted before a prescript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathspacebetweenscript}}]

This is the horizontal space inserted between a super- and subscript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathstackdenomdown}}]

This is the downward shift of the denominator in an \prm {atop} stack.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathstacknumup}}]

This is the upward shift of the numerator in an \prm {atop} stack.
...
This is the vertical clearance between the elements of an \prm {atop} stack.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsubscriptsnap}}]

This controls snapping of small standalone subscript boxes to the normal script
position.
...
This is the downward shift of a subscript attached to a character.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsubshiftdrop}}]

This is the subscript drop used for boxes and subformulas.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsubsupshiftdown}}]

This is the subscript shift used when a superscript is present as well.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsubsupvgap}}]

This is the vertical clearance between a superscript and a subscript.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsubtopmax}}]

This limits how high a standalone subscript may reach above the baseline.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsupbottommin}}]

This sets the minimum height of the bottom of a standalone superscript above the
baseline.
...
This controls snapping of small standalone superscript boxes to the normal script
position.
...
This is the superscript drop used for boxes and subformulas.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsupshiftup}}]

This is the upward shift of a superscript attached to a character.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathsupsubbottommax}}]

This limits how low the superscript of a combined super- and subscript may reach.
...
This is the vertical clearance below the rule in an underbar.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathunderbarrule}}]

This is the thickness of the rule in an underbar.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathunderbarvgap}}]

This is the vertical clearance above the rule in an underbar.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathunderdelimiterbgap}}]

This is the baseline clearance used by \prm {Uunderdelimiter} and \prm
{Udelimiterunder}.
...
The current value of this parameter is \CurrentMathVariant
\Umathunderdelimitervariant. It is used by \prm {Uunderdelimiter}.

\stopnewprimitive

\startnewprimitive[title={\prm {Umathunderdelimitervgap}}]

This is the vertical clearance used by \prm {Uunderdelimiter} and \prm
{Udelimiterunder}.
...
This modifier aligns the current math atom on the math axis.
...
This modifier discards the content and dimensions of the current math atom.
...
This constructs a large operator from a \UNICODE\ math character. It uses the
same options and sizing mechanism as the extended fences.
...
This command is one of the radical constructors. A radical has a delimiter on
the left and a radicand. A degree and a right delimiter are available in the
related constructors. The delimiter can be a variant glyph or an extensible
made from several glyphs.

The basic forms are:

\starttabulate[|l|l|]
    \NC \type {\Uradical} \NC <left delimiter> {<radicand>} \NC \NR
    \NC \type {\Uroot}    \NC <left delimiter> {<degree>} {<radicand>} \NC \NR
    \NC \type {\Urooted}  \NC <left delimiter> <right delimiter> {<degree>} {<radicand>} \NC \NR
\stoptabulate

The \prm {Udelimited} variant takes two delimiters but no degree. The
\prm {Uoverdelimiter} and \prm {Uunderdelimiter} variants put an extensible
delimiter above or below a formula, while \prm {Udelimiterover} and \prm
{Udelimiterunder} put the formula above or below an extensible delimiter. The
\prm {Uhextensible} variant constructs only the horizontal extensible.

These commands accept options for dimensions, style, delimiter alignment,
overflow and rule handling. The options are \type {attr}, \type {bottom},
\type {callback}, \type {depth}, \type {exact}, \type {height}, \type {left},
\type {middle}, \type {norule}, \type {nooverflow}, \type {right}, \type
{reflected}, \type {shrink}, \type {size}, \type {stretch}, \type {style},
\type {source}, \type {symbolattr}, \type {top}, \type {usecallback} and
\type {width}. Not all options apply to every constructor. The parameters that
control these constructions are summarized in the appendix.

The radical primitives share a construction path, but not every parameter is
read by every variant. The following table gives the main relationships. The
parameters ending in \type {variant}, and the \type {extensible...} parameters,
are \LUAMETATEX\ additions; the other radical and delimiter parameters are
based on the \OPENTYPE\ math parameters.

\starttabulate[|l|p|p|]
    \FL
    \BC parameter \BC use \BC constructors \NC \NR
    \TL
    \NC \type {Umathradicalvgap}
        \NC clearance between the radicand and the radical rule
        \NC \type {\radical}, \type {\Uradical}, \type {\Uroot}, \type {\Urooted} \NC \NR
    \NC \type {Umathradicalkern}
        \NC clearance above the radical rule
        \NC \type {\radical}, \type {\Uradical}, \type {\Uroot}, \type {\Urooted} \NC \NR
    \NC \type {Umathradicalrule}
        \NC radical rule thickness, also used as callback line width
        \NC radical and delimiter forms, except \type {\Uhextensible} \NC \NR
    \NC \type {Umathradicaldegreebefore}, \type {Umathradicaldegreeafter}
        \NC horizontal kerns before and after the degree
        \NC \type {\Uroot}, \type {\Urooted} \NC \NR
    \NC \type {Umathradicaldegreeraise}
        \NC percentage used for the vertical position of the degree
        \NC \type {\Uroot}, \type {\Urooted} \NC \NR
    \NC \type {Umathradicalextensiblebefore}, \type {Umathradicalextensibleafter}
        \NC horizontal space around an extensible radical symbol
        \NC \type {\radical}, \type {\Uradical}, \type {\Uroot}, \type {\Urooted} \NC \NR
    \NC \type {Umathradicalvariant}
        \NC style used for the radicand
        \NC radical forms and \type {\Udelimited} \NC \NR
    \NC \type {Umathoverdelimiterbgap}, \type {Umathoverdelimitervgap}
        \NC baseline and vertical clearance for an upper delimiter
        \NC \type {\Uoverdelimiter}, \type {\Udelimiterover} \NC \NR
    \NC \type {Umathunderdelimiterbgap}, \type {Umathunderdelimitervgap}
        \NC baseline and vertical clearance for a lower delimiter
        \NC \type {\Uunderdelimiter}, \type {\Udelimiterunder} \NC \NR
    \NC \type {Umathdegreevariant}
        \NC style used for the degree
        \NC \type {\Uroot}, \type {\Urooted} \NC \NR
    \NC \type {Umathhextensiblevariant}
        \NC style used for the horizontal extensible
        \NC \type {\Uhextensible} \NC \NR
\stoptabulate

The radical rule is made directly for the usual radical forms. In the related
delimiter forms it is also passed to an extensible callback as the requested line
width. The horizontal-extensible form instead uses \prm {Umathfractionrule} for
that callback.
...
This constructs a vertically extensible math symbol around the following
material. It accepts the same sizing options as the other extended fences.
...
This expands to the math style currently in effect, including its cramped state.
Unlike an explicit style command, it does not add a style node to the math list.
...
This bitset controls which kinds of hyphenation and discretionary injection are
enabled. The individual bits are described in the language documentation.
...
This is a variant of \prm {ifcase} where the selector is the number of arguments
...
When optional arguments are represented by delimiters in the macro definition,
this primitive stops scanning at the current point. Any remaining arguments are
left empty and can be handled by the macro body.
...
This sets the penalty used for a break in the current paragraph when the local
paragraph settings are active.
...
This sets the interline penalty for the current paragraph without changing the
global \prm {interlinepenalty}.
...
This sets the first-pass line-breaking tolerance for the current paragraph
without changing the global \prm {pretolerance}.
...
This sets the line-breaking tolerance for the current paragraph without changing
the global \prm {tolerance}.
...
The alignment options can be changed for the following row with \prm
{alignoption}. It accepts \type {firstskip}, \type {lastskip}, \type
{nofirstskip}, \type {nolastskip} and \type {prune}.
...
You can set this register with options for the upcoming paragraph. Of course you
have to make sure that it indeed concerns that paragraph and not one that gets
rendered as side effect before that paragraph is typeset. \stopnewprimitive
...
This primitive does a preroll of a specific balancing stream. It returns a list
with empty lines. Its argument in an index.
...
\startnewprimitive[title={\prm {romannumerical}}]

This is a variant of \prm {romanumeral} that doesn't suffer from the excessive
generation of \type {m}'s when we cross the 4000 boundary, so \im {1234567} will
give you {\tttf \romannumerical {1234567}} instead of \ctxlua { context
(\letterhash "\normalromannumeral1234567" - # "dlxvii" ) } {\tttf m}'s in front
of {\tttf dlxvii}.
...
This tracker keep track of how ragged a paragraph is. It relates to \prm
{justificationskip}.
...
\doif {\inputfilename} {luametatex-primitives.tex} {
