...
Note 2: Careful readers will realize that there are also primitives for the items
...
There are fundamental differences in how traditional \TEX\ uses italic correction
and how \OPENTYPE\ sees it. Unfortunately after decades of \LUATEX\ we are in a
situation where many math fonts (at least those originating from the \TEX\ world)
use the old model. For that reason we eventually decided to let that one dominate
even if it gives bad results. Large operators are troublemakers so we have a
parameter to control them: setting \type {\mathlargeoperatormode} to a positive
value enforces the traditional path (with zero being the default). Of course
macro packages can (and likely have to) deal with these matters differently.
